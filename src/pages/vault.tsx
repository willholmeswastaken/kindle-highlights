...
const ImportHistory: NextPage = () => {
    const { data: vaultRecords } = trpc.useQuery(["vault.get"]);
    return (
        <>
            <Head>
                                    ...
                                    return <Link key={x.id} href={`/view-import/${x.id}`}>
                                        <a className="flex flex-row bg-white rounded-lg hover:bg-gray-50 h-16 border-y border-y-gray-100 py-3 px-2 hover:cursor-pointer" key={x.id}>
                                            ...
                                            <div className="flex flex-col pl-2 flex-grow">
                                                <span className="text-brandText hidden md:block capitalize">{formatRelative(x.savedOn, new Date())}</span>
                                                <span className="text-sm text-brandText md:hidden">{formatDistance(x.savedOn, new Date(), { addSuffix: true })}</span>
